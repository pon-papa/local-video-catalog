...
| VLM モデル | `/models` の一覧から `model_match` を一意解決。曖昧なら**停止**（勝手に選ばない） | 「指定モデルが見つかりません。ローカルAI設定で選び直してください」 |
| VLM の画像入力対応 | **その場で生成した単色 PNG** を 1 枚送って確認（ユーザーの画像は使わない） | 「このモデルでは画像入力を確認できません」 |
...
| 14 | 実行条件 | 稼働時間・本数上限・制限なし・映像解析を飛ばす・完了後に中間ファイル整理 |
